...
class IBKRClient:
    ...
    async def _set_market_data_type(self, data_type: int):
        ...
        if self._current_market_data_type != data_type:
            ...
            logger.info(f"Set market data type to {data_type}")
    ...
    async def get_multiple_market_prices(self, symbols: List[str]) -> Dict[str, float]:
        ...
        async with self._market_data_lock:
            ...
            try:
                ...
                logger.info(f"Live data failed: {e}")
            ...
            if remaining_symbols:
                try:
                    ...
                    logger.info(f"Frozen data failed: {e}")
            ...
            if remaining_symbols:
                try:
                    ...
                    logger.info(f"Delayed data failed: {e}")
            ...
            if remaining_symbols:
                try:
                    ...
                    logger.info(f"Historical data failed: {e}")
            ...
            for symbol, ticker in tickers.items():
                try:
                    ...
                    if price and price > 0:
                        ...
                        logger.warning(f"No valid {data_type_name} price data available for {symbol}")
                        continue
                        
                except Exception as e:
                    logger.warning(f"Failed to get {data_type_name} price for {symbol}: {e}")
        ...
        async with self._order_lock:
            ...
            try:
                ...
                if trade.orderStatus.status == 'Cancelled':
                    ...
                    if trade.log:
                        for log_entry in trade.log:
                            if log_entry.errorCode and log_entry.errorCode > 0:
                                ...
                                logger.error(f"WhatIf order for {symbol} rejected: {log_entry.message}")
                                return {
                                    'valid': False,
                    ...
                    logger.info(f"WhatIf order for {symbol} validated successfully")
                    return {
                        'valid': True,
                        'error': None,
    ...
    async def _wait_for_orders_filled(self, account_id: str, order_ids: List[str], max_wait_seconds: int = 300):
        ...
        while True:
            ...
            if not pending_orders:
                logger.debug(f"All specified orders completed for account {account_id}")
                return
