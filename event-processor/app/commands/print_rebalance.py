...
class PrintRebalanceCommand(EventCommand):
    ...
    async def execute(self, services: Dict[str, Any]) -> EventCommandResult:
        ...
        event_logger.log_info(f"Printing rebalance orders for account {self.event.account_id} (dry run)", self.event)
        ...
        try:
            ...
            if not result.orders:
                event_logger.log_info(f"No rebalance orders needed for account {self.event.account_id}", self.event)
            else:
                event_logger.log_info(f"Rebalance orders for account {self.event.account_id} (would execute {len(result.orders)} orders):", self.event)
                
                for order in result.orders:
                    event_logger.log_info(f"  Would {order.action} {order.quantity} shares of {order.symbol} (${order.market_value:.2f})", self.event)
            ...
            event_logger.log_error(f"Print rebalance failed: {e}", self.event)
